...
public class CabServiceImpl implements CabService {
	...
	public Cab updateCab(Cab cab) throws ResourceNotFoundException {
		...
		if (optionalCab.isEmpty()) {
			throw new ResourceNotFoundException("No cab existing with id: " + cab.getCabId());
		}
	...
	public void deleteCab(int cabId) {
		...
		if (optionalCab.isEmpty()) {
			throw new ResourceNotFoundException("No cab existing with id: " + cabId);
    ...
    public List<CabResponse> getAllCabs() {
        ...
        for (Cab cab : cabList) {
            ...
            cabResponse.setDriverId(cab.getDriverId());
				...
				try {
					Driver driver = driverService.getDriverDetails(cab.getDriverId());
